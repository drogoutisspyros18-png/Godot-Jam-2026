...
func add_collision_polygon(polygon_2d: Polygon2D):
	...
	static_body_2d.set_meta("destruct_root", self)
...
func _update_or_create(polygon_2d, polygon, size, new):
	...
	if new:
		...
		# polygon_2d.modulate = Color(randf(), randf(), randf())[cite: 4]

		# FIX 1: If duplicate() stripped away our un-owned physics nodes, rebuild them manually.
		if _has_been_collidable and polygon_2d.get_child_count() == 0:
			add_collision_polygon(polygon_2d)
	...
	if _has_been_collidable:
		...
		var col_poly = polygon_2d.get_child(0).get_child(0)
		col_poly.set_deferred("polygon", polygon)
